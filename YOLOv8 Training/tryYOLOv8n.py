...
class Red_Square_Detection:
    def __init__(self, model_path, video_source):
        self.model = YOLO(model_path)
        # self.model.overrides['conf'] = 0.6
        self.model.overrides['max_det'] = 1
        self.model.overrides['iou'] = 0.1
        self.model.overrides['device'] = 'cuda:0'
        # self.model.half = True # half precision FP16 inference for faster performance but lower accuracy
        # self.model.overrides['vid_stride'] = 4 # video frame stride to reduce processing (default is 1)
        self.model.overrides['classes'] = [1]
    ...
    def show_class_onFrame(self, frame, class_names):
        if class_names:
            cv2.putText(frame, "square" if class_names[0] == 1.0 else "none", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 225, 0), 2, cv2.LINE_AA)
        else:
...
# Usage
model_path = 'red_square_yolov8n.pt'
video_source = 0
axe_detection = Red_Square_Detection(model_path, video_source)
axe_detection.run()